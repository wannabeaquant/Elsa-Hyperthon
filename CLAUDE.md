...
**What:** An autonomous AI trading agent that buys crypto dips on Base network, powered by Claude Sonnet 4.6 using the Claude Agent SDK + Elsa x402 micropayment protocol.
...
1. Claude reads memory.json                        (free — local file)
...
7. Claude reasons with conviction scoring:
...
10. Claude writes memory.json                      (free — local file)
    ↓ (snapshot, trade record, price obs, signals, conviction, seasonality)
```

**Key insight:** Claude makes all decisions. The system prompt describes *goals and constraints*, not *steps*. Claude decides which tokens to check, when to trade, how much, and when to sell — narrating every cost decision out loud for demo visibility.
...
### Main Agent Loop
- **`src/agent.ts`** — The heart. Uses `query()` from `@anthropic-ai/claude-agent-sdk`. Contains:
...
### 4. OAuth Token
- Get `CLAUDE_CODE_OAUTH_TOKEN` from Claude Code settings (Settings → Account)
- Fill into `.env`
- Enables Claude Agent SDK using your Claude Max/Pro subscription — no per-token billing
...
Before spending a cent on API calls, Claude runs `src/helpers/analytics.ts` — a free local script that reads `memory.json` and pre-computes:
...
5 signals Claude checks per token:
- ETH-adjusted dip ≤ -5%
...
- **Agent notes** — freeform pattern observations Claude writes itself

Over time memory becomes the agent's "experience" — win rates per dip magnitude, recovery times per token, best trading hours.

### WETH Baseline

Every cycle (when gas permits), Claude checks WETH price *before* any memecoins. This sets `_lastWethChange` in display.ts, which is then used to label every subsequent token price result with its ETH-adjusted dip. Visible in terminal as `ETH-adj: -7.2% ← REAL DIP`.

### Cost Narration (Critical for Demo)

Claude speaks every spending decision out loud:
...
Claude doesn't see any of this — it just calls `npx tsx src/helpers/gas.ts` and gets JSON back.
...
### Why Claude Sonnet 4.6 (not Opus)?
...
- OAuth approach: flat $20/month Claude Max subscription, no per-token billing
...
- Simple permission model: Claude runs tools via Claude Code Bash

### Why memory.json instead of vector DB?
- Local JSON, no external dependencies, zero setup
- Claude reads/writes readable JSON — easy to inspect, debug, and pre-seed
- For a hackathon: transparency beats sophistication

### Why goal-based prompt over step-by-step script?
- Flexibility: Claude adapts (checks gas before prices, skips flat tokens)
- Intelligence: Claude weighs tradeoffs ("is -4% dip worth 0.08 gwei gas?")
...
**Goal:** Show judges that Claude is making real, intelligent trading decisions on Base using Elsa's x402 micropayment protocol — with every cost decision narrated out loud.
...
6. Read Claude's reasoning out loud — it narrates costs, skips, and decisions
...
**Talking points:**
- "Claude reads its own memory first — it knows what worked last time"
